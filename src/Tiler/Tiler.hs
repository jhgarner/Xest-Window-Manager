        ...
        case predicateMove $ map fst t of
          ...
          Just functions@(_, unparented) ->
            if any (isUnmovable . snd) t
              then -- If it already contained the unmovable part, we're done!
                (withNewFocus t, Both)
              else -- Otherwise, let's remove it and set the right flag
                (unparented, Movable functions)
...
getBorders =
  fromMaybe (error "No IC here!") . cata \case
