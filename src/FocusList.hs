...
push dir foc a FL {visualOrder = vo, focusOrder = fo, actualData = ad} =
  FL {visualOrder = dirSide, focusOrder = focSide, actualData = ad ++ [a]}
  where focSide = if foc == Focused then len:fo else fo ++ [len]
...
pop (Right Focused) fl@FL {visualOrder = vo, focusOrder = fo, actualData = ad} = 
  fromMaybe (Nothing, fl) $ do
    ...
    top <- index ad loc
    tailFo <- tailMay fo
    let popped = FL {visualOrder = remove loc vo, focusOrder = tailFo, actualData = removeI loc ad}
...
pop (Right Unfocused) fl@FL {visualOrder = vo, focusOrder = fo, actualData = ad} =
  fromMaybe (Nothing, fl) $ do
    ...
    top <- index ad loc
    initFo <- initMay fo
    let popped = FL {visualOrder = remove loc vo, focusOrder = initFo, actualData = removeI loc ad}
    return (Just top, reduce loc popped)
...
pop (Left Front) fl@FL {visualOrder = vo, focusOrder = fo, actualData = ad} =
  fromMaybe (Nothing, fl) $ do
    ...
    top <- index ad loc
    tailVo <- tailMay vo
    let popped = FL {visualOrder = tailVo, focusOrder = remove loc fo, actualData = removeI loc ad}
    return (Just top, reduce loc popped)
...
pop (Left Back) fl@FL {visualOrder = vo, focusOrder = fo, actualData = ad} =
  fromMaybe (Nothing, fl) $ do
    ...
    top <- index ad loc
    initVo <- initMay vo
    let popped = FL {visualOrder = initVo, focusOrder = remove loc fo, actualData = removeI loc ad}
    return (Just top, reduce loc popped)

reduce :: Int -> FocusedList a -> FocusedList a
reduce removed fl@FL {..} = fl {focusOrder = newL focusOrder, visualOrder = newL visualOrder}
...
mapOne (Right Focused) f fl@FL {focusOrder = mfo, actualData = ad} = 
  fl {actualData = maybe [] (\fo -> map (\(i, a) -> if i == fo then f a else a) $ zip [0..] ad) $ headMay mfo}

mapOne (Right Unfocused) f fl@FL {focusOrder = mfo, actualData = ad} = 
  fl {actualData = maybe [] (\fo -> map (\(i, a) -> if i == fo then f a else a) $ zip [0..] ad) $ lastMay mfo}

mapOne (Left Front) f fl@FL {visualOrder = mvo, actualData = ad} = 
  fl {actualData = maybe [] (\vo -> map (\(i, a) -> if i == vo then f a else a) $ zip [0..] ad) $ headMay mvo}

mapOne (Left Back) f fl@FL {focusOrder = mvo, actualData = ad} = 
  fl {actualData = maybe [] (\vo -> map (\(i, a) -> if i == vo then f a else a) $ zip [0..] ad) $ lastMay mvo}
...
flFilter predicate FL {actualData = ad, visualOrder = vo, focusOrder = fo} =
  FL {actualData = filter predicate ad, visualOrder = foldl' removeFrom vo gone, focusOrder = foldl' removeFrom fo gone}
  where gone = foldl' (\acc (i, a) -> if predicate a then acc else i:acc) [] $ zip [0..] ad
...
vOrder FL {visualOrder = vo, actualData = ad} = map (fromMaybe (error "focList broken") . index ad) vo

focusElem :: Eq a => a -> FocusedList a -> FocusedList a
focusElem a fl@FL {focusOrder = fo, actualData = ad} = 
  fl {focusOrder}
...
focusIndex i fl@FL {focusOrder = fo} = 
  fl {focusOrder = if length fo > i then newFo else fo}
...
makeFL actualData focIndex = 
  FL {visualOrder = vo, focusOrder = fo, actualData}
    where len = length actualData
          vo = [0..len - 1]
          fo = focIndex : filter (/= focIndex) vo

emptyFL :: FocusedList a
emptyFL = 
  FL {visualOrder = [], focusOrder = [], actualData = []}

focusDir :: Direction -> FocusedList a -> FocusedList a
focusDir Front fl@FL {focusOrder = fo, visualOrder = vo} = 
  fromMaybe fl $ do
...
focusDir Back fl@FL {focusOrder = fo, visualOrder = vo} = 
  fromMaybe fl $ do
    currentFoc <- headMay fo
