...
reduce (Directional d fl) = 
  if isEmpty newTiler then EmptyTiler else newTiler
...
placeWindows (Directional d fl) oldR@Rect {..}
  = foldl' (\acc (i, f) -> acc >> f (location d i)) (pure ()) $ zip [0..] $ vOrder fl
 where
  ...
  location Z i = if i == fromIntegral (fromMaybe 1000 (getFocIndex fl)) then oldR else Rect 0 0 0 0
...
modFocused f (Directional d fl) = 
  Directional d $ mapOne (Right Focused) f fl
...
focus newF (Directional d fl) = 
  Directional d $ focusElem newF fl
...
focusWindow
  :: Window -> Tiler (Tiler (Fix Tiler), (Bool, Bool)) -> (Tiler (Fix Tiler), (Bool, Bool))
